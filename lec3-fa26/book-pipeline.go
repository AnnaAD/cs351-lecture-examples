...
// Global DB instance so the example can call DB.get_books(...) directly.
var DB = NewDatabase()
...
// get_all_ratings returns numReadings fake ratings for the given books.
// Sleeps to simulate a slow DB query.
...
func main() {
	...
	booksChan := make(chan ArgsBook)
	profileChan := make(chan ArgsProfile)
	go func() {
		for _, user := range request_queue {
			log.Println("Getting Books: ", user)
			books := DB.get_books(user)
			booksChan <- ArgsBook{books:books, user:user}
		}
		close(booksChan)
	}()
	...
	go func() {
		for b := range booksChan {
			log.Println("Getting Ratings: ", b.user)
			ratings := DB.get_all_ratings(b.books)

			log.Println("Building Profile: ", b.user)
			profile := build_profile(ratings)
			profileChan <- ArgsProfile{profile:profile, user:b.user}
		}
		close(profileChan)
	}()
	
	for p := range profileChan {
		log.Println("Getting Recs: ", p.user)
		rec_books := get_recommendations(p.profile)
		
		log.Println("Books Recommended:", p.user, len(rec_books))
	}
